...
内部辅助节点 `DAELAB.BadgeApp87RegionAlignV1` 输入源 IMAGE、生成后的 IMAGE 和源 MASK，输出目标 MASK、保色参考 IMAGE 和配准 STRING 报告。它没有新增 APP 控件，统一接入共享 Bypass 行为。配准保留原图朝向，避免圆形轮廓的旋转歧义；32 像素内缩种子和边界吸附用于修正生成图的边界偏移。大幅旋转、形变或细窄区域仍需要人工检查。画笔底图自动跟随编辑目标；加载未完成时执行入口提示等待，Load Image 保留为手动重试。
