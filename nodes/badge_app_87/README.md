...
“待编辑目标图”顶部同样提供“使用生成结果 / 已有效果图”来源胶囊，默认使用生成结果。两种图片分别保存：选择生成结果时跟随最新效果图；选择已有效果图时可上传或从图库选择，新生成不会覆盖当前手动目标。切换来源会同步预览和实际编辑输入，并使旧选区预览失效；没有生成结果时显示明确空状态。

重启 ComfyUI 加载新节点，打开 #8.7，进入 APP Mode，使用原有的“效果图生成”和“局部修改”页签。模型任务使用 ComfyUI 已登录账户；测试默认 Low、1:1、1024×1024、1 张。

两个页签的“模型输出配置”均支持 1–8 张生成数量，与本机 `OpenAIGPTImageNodeV2` 的 `n` 范围一致，默认 1 张，分别保存。基础调用直接传递 `n`；局部保护和区域材质逐张处理，再合并为完整输出批次。“使用生成结果”默认采用批次中的第一张，全部图片仍保留在原生输出区。选择数量不改变底部的运行次数。
...
- 材质分区：复用材质目录及提示词，按源图颜色生成选区；由 `DAELAB.BadgeApp87RegionAlignV1` 复用现有 render-space 配准与边界吸附处理，将选区映射到实际生成图。区域材质依次生成，复用 `BadgeMaterialConstraintV1` 保色/材质固有色策略，最后确定性合成。配准 IoU 低于 0.65 时停止区域生成，避免把材质应用到错误位置。
- 局部修改：使用独立参考图；原图或 Color ID Map 取色，以及 Polygon/画笔均复用原型操作。先在界面预览遮罩，再点击生成。服务端复核当前源图和选区，才执行语义修改或材质转换；选区外像素严格保留。
- 输出图和任务记录进入 ComfyUI 原生输出区。效果图生成成功后更新“使用生成结果”来源；当前选中该来源时同步更新局部目标并使旧选区失效，无需下载后重新上传。选择“已有效果图”时保留手动目标，局部生成结果也不会自动覆盖当前目标图。

原型实际展示的两个页签保持不变；保留的棚拍节点没有新增可见入口。适配器的 `studio` 支持单独集成测试，不代表增加了棚拍界面。

## 接口与复用

输入 `request_json`（STRING）由 #8.7 前端编译，包含阶段、稳定图片引用、当前有效选区/材质/高度配置和输出尺寸；节点输出 IMAGE 和 STRING 报告。局部修改的 `apply` 必须携带同一服务端会话最近预览得到的 `preview_token`。改变图片内容、选区、提示词、尺寸或服务端重启后均需重新预览。

内部辅助节点 `DAELAB.BadgeApp87RegionAlignV1` 输入源 IMAGE、生成后的 IMAGE 和源 MASK，输出目标 MASK、保色参考 IMAGE 和配准 STRING 报告。它没有新增 APP 控件，统一接入共享 Bypass 行为。配准保留原图朝向，避免圆形轮廓的旋转歧义；32 像素内缩种子和边界吸附用于修正生成图的边界偏移。大幅旋转、形变或细窄区域仍需要人工检查。画笔模式保留原有的 Load Image 操作；如果画布仍显示旧图，执行入口要求先加载当前目标图再预览。
