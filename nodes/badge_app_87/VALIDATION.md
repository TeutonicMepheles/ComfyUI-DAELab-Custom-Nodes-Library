...
## 2026-09-11 刷新后局部参考图媒体告警修复

- 复现：实际 #8.7 工作流的 `Badge87/build_00040_.png [output]` 引用有对应磁盘文件，局部大图能够显示，但原生 App Mode 报媒体缺失。上一轮只验证了选图/预览，没有闭环此告警。
- 原因：目标绑定只调用 image widget 的预览 callback，未触发原生 `node.onWidgetChanged(name, value, previous, widget)`；同图恢复还提前返回，无法同步原生 widget 错误状态。原生前端的 widget 变更钩子负责清理对应媒体错误。
- 修复：统一原生绑定通知；同图恢复也修复候选值和命名序列化值，但保留选区、预览及校验状态；空来源不发送清除媒体错误的通知。没有修改上游前端或直接清理错误 store。
- 验证：275 项前端测试通过，新增同图重开、候选列表/命名值恢复、原生通知及空来源回归。在新浏览器页面打开同一 #8.7、进入局部修改后，原图恢复且媒体告警消失，等待图片加载和阶段往返后仍未出现。未提交模型任务。

## 2026-09-11 多生成结果选择

- 保留原有阶段执行、来源胶囊、LoadImage、共享 Bypass、自动遮罩校验和原生输出区；新增整批结果单选条与逐图草稿。首次默认第 1 张，后续批次等待用户选择，不替换当前目标。
- `node --test tests/*.test.mjs`：272 项通过。新增覆盖整批选择、同图重复选择不失效、新批次保留当前目标、忙碌锁定、来源/选图序列化、颜色/分区图归属、Polygon/画笔草稿与快速切图的加载令牌递增。包含共享 App Mode Bypass 测试。
- 在本机 `127.0.0.1:8000` 的实际 App Mode 中，用现有输出图建立独立临时工作流，验证 8 张缩略图加载、选择后目标文件名同步、上传/生成来源往返、阶段页签往返保留选中图。第 1 张 Reset 创建 Polygon 后，第 2 张 Clear 禁用；切回第 1 张 Clear 恢复可用，证明独立选区恢复。
- 初版双行布局过高，改为紧凑单行条。1280px 视口下选择区宽约 571px、高 115px；390px 视口下宽 341px、高 130px，条内滚动宽 554px，overflow-x:auto，第 8 张可通过键盘选择。保留原参考图区高度调节，不修改节点尺寸或工作流布局。
- 浏览器重新打开页面后确认加载新版并保留选中图；同页普通刷新未可靠重新加载，不能据此声称连续刷新验证通过。相关执行/目标/选区模块入口已更新版本参数。
- 本轮未提交新的模型生成任务；新批次完成接入与忙碌状态由回归测试验证。未补做真实模型生成到局部编辑的完整端到端验收，也未补做实际 Muted/Bypassed DOM 往返测试。临时测试工作流未覆盖正式模板。
