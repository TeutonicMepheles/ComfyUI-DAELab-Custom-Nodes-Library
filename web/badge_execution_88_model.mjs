...
import { isBadge88, localMaterialNodeId88, usesLocalMaterials88 } from './badge_local_material_88_model.mjs?v=20260911-88-1';
...
export function requestForStage(graph, stage) {
    if (!isBadge88(graph)) throw new Error('Not a Badge 8.8 workflow.');
    ...
    if (multi) {
        ...
        if (!config?.groups?.length) throw new Error('请添加至少一个局部材质区域。');
...
export function promptForRequest(request, executorId = 200) {
    ...
    prompt[executorId]._meta.title = '#8.8 阶段执行';
    prompt['113']._meta.title = '#8.8 生成结果';
