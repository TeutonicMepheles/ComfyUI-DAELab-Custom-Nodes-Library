...
import {
    ...
    removeLastGroup,
    updateGroup,
} from "./multi_color_mask_model.mjs?v=20260819-1";

const NODE_TYPE = "DAELabMultiColorMask";
const CONFIG_PROPERTY = "multi_color_mask_config";
...
function markDirty(node, { refreshInspector = true } = {}) {
    if (node.setSize && node.computeSize) {
        ...
        const savedWidth = Number(node.properties?.multi_color_mask_width);
        ...
        node._multiColorMaskAutoSizing = true;
        try {
            node.setSize([width, computed[1]]);
        } finally {
            node._multiColorMaskAutoSizing = false;
        }
    }
...
function removeDynamicWidgets(node) {
    removeOwnedWidgets(node, "_daelabMultiColorMask");
}
...
function addDynamicWidget(node, type, name, value, callback, options = {}) {
    ...
    widget._daelabMultiColorMask = true;
...
function addSectionLabel(node, label) {
    const widget = {
        name: `section_${label}`,
        type: "daelab-multi-color-mask-section",
        value: "",
        _daelabMultiColorMask: true,
        serialize: false,
...
function managedWidgetName(name) {
    return name === "output_mask" || /^(?:enabled|color|threshold|invert)_\d+$/.test(String(name));
}
...
function removeStaleAppModeInputs(node, config) {
    ...
    const valid = new Set(["output_mask"]);
    config.groups.forEach((_group, index) => {
        const number = index + 1;
        for (const prefix of ["enabled", "color", "threshold", "invert"]) {
...
function syncOutputLabel(node, output) {
    ...
    socket.name = "mask";
    socket.type = "MASK";
    socket.label = output === COMBINED_OUTPUT ? "combined_mask" : output;
    socket.localized_name = socket.label;
}

function rebuildUI(node) {
    if (node._multiColorMaskBuilding) return;
    node._multiColorMaskBuilding = true;
    try {
        let config = setConfig(node, getConfig(node));
        removeDynamicWidgets(node);

        addDynamicWidget(node, "button", "Add Mask Group", null, () => {
        ...
        addDynamicWidget(
            ...
            config.output,
            (value) => {
                const next = getConfig(node);
                next.output = value;
                config = setConfig(node, next);
                syncOutputLabel(node, config.output);
                markDirty(node);
            },
            { values: outputChoices(config.groups.length) }
        );

        config.groups.forEach((group, index) => {
            const number = index + 1;
            addSectionLabel(node, `Mask ${number}`);
            ...
            addDynamicWidget(node, "toggle", `invert_${number}`, group.invert, (value) => {
                config = setConfig(node, updateGroup(getConfig(node), index, { invert: value }));
                markDirty(node);
            });
        });
        ...
        node._multiColorMaskBuilding = false;
    }
}
...
app.registerExtension({
    ...
    beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        chainCallback(nodeType.prototype, "onResize", function (size) {
            ...
            if (!this._multiColorMaskAutoSizing && Number.isFinite(width) && width >= DEFAULT_WIDTH) {
                this.properties ||= {};
                this.properties.multi_color_mask_width = width;
            }
        });
        ...
        nodeType.prototype.getExtraMenuOptions = function (_, options) {
            ...
            options.unshift(
                {
                    content: `Add Mask Group (${getConfig(this).groups.length}/${MAX_MASK_GROUPS})`,
                    callback: () => {
                ...
                {
                    content: "Remove Last Mask Group",
                    callback: () => {
